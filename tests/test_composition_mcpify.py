...
mcpify's stderr is captured to a file and attached to any failure —
a server that dies at startup must not die silently twice.
"""
...
def _paced_talk(session: Path, api_spec: Path, err_path: Path) -> None:
    err = err_path.open("w+")
...
def test_composition_mcpify_through_the_tap(tmp_path, fixture_api):
    ...
    _paced_talk(session, api_spec, tmp_path / "mcpify.err")
